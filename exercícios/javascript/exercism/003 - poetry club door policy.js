...
// Because there have been incidents in the past, the club has a very
// specific door policy which you'll need to master, before attempting entry.

// There are two doors at the poetry club, a front and a back door, and both
// are guarded. To gain entry, you'll need to work out the password of the day.
...
// acrostic poems. This means that the first letter of each sentence forms a word.
...
// So when the guard recites Stands so high, you'll respond S, when the guard recites
// Huge hooves too, you'll respond H.

// Implement the function frontDoorResponse that takes a line of the poem as an argument
// and returns the first letter of that line.
...
// Implement the function frontDoorPassword that accepts a string (the combined letters
// you found in task 1) and returns it correctly capitalized.
...
// To determine letters for the back door password, you need to respond with the last letter
// of the line of the poem that the guard recites to you.
...
// When the guard recites Stands so high, you'll respond h, when the guard recites Huge hooves
// too, you'll respond o.

// Note that sometimes the guard does stylist pauses (in the form of whitespace) at the end of
// a line. You will need to ignore those pauses to derive the correct letter.
...
const lastLetter = lineAux => lineAux[lineAux.length - 1]

function backDoorResponse(line) {
  return lastLetter(line.trim())
}
